...
def markAsRead(msgid):
    #res = msgs.get(id=msgid).execute()
    print('attemp to set unread=false TODO' + res)  

def forSelfAutomation(id, subj):
    print('forSelfAutomation>'+subj)
    if 'car=' in subj:
        try:
            spec = subj.split('car=')[1]
            carname = spec.split(' ')[0]
            with open(carname+'.csv', 'w') as carfile:
                carfile.write(spec+'\n')
            markAsRead(id)    
        except:
            print('could not add car data for '+spec)
    ...
    if notFound:
        if (is_valid_info(snip)):
            try:
                ...
                print('Event Could not be created for:' + snip)            
        else:
    ...
    if not labels:
        ...
        for label in labels[:20]:
            ...
            for f,s in zip(froms,subjs):
                if 'three6tdegree@gmail' in f:
                    if 'uto ' in s:
                        forSelfAutomation(id,s)                
                if '@guru' in f:
                    print('!', end='')
                    guru.append(id)
                if 'Bank' in s:
