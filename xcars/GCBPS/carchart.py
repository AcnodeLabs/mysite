...
def getPrices(f, brand):
    ...
    i = 0
    for price in prices:
        f.write(names[i] + "," + price+ ',$'+ str(int(float(price)/usd))+ ',24k10g '+ str(int(float(price)/gold))+'\n')
        i=i+1
...
# CAR CAHRT 
def getFileNamePr(yr,months):
    for m in months:
        spec = 'carprices-' + str(yr) + "_" + str(m) + '_*.csv'
...
def carchart():
    fn = getFileNamePr(2020,availmonthrange())
# car chart
