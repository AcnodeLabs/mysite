...
function popularfx_import_template($slug){
...
function popularfx_start_install_template(){
	...
	jQuery.ajax({
		...
		error: function(){
			popularfx_show_error({err: 'AJAX failure !'});
		}
	});
	...
	jQuery.ajax({
		...
		success:function(theme) {
			...
			for(var x in theme['screenshots']){
				sc += '<div class="popularfx_img_screen" page="'+x+'">'+
					'<div class="popularfx_img_div"><img src="'+url+'/screenshots/'+theme['screenshots'][x]+'" width="100%" /></div>'+
					'<div class="popularfx_img_name">'+strip_extension(theme['screenshots'][x])+'</div>'+
				'</div>'
			}
